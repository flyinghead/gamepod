...
int timeout_passed(const int timeout)
{
    if (timeout >= 0 && timeout < TIMEOUTS) {
        const int  state = timeout_state[timeout];
...
int timeout_unset(const int timeout)
{
    if (timeout >= 0 && timeout < TIMEOUTS) {
        ...
        const int  state = timeout_state[timeout];
	timeout_state[timeout] &= TIMEOUT_PASSED;
...
int timeout_set(const double seconds)
{
    ...
    for (timeout = 0; timeout < TIMEOUTS; timeout++)
        if (!(timeout_state[timeout] & TIMEOUT_USED))
            break;
    ...
    timeout_state[timeout] |= TIMEOUT_ARMED;

    /* How long till the next timeout? */
    next = seconds;
    for (i = 0; i < TIMEOUTS; i++)
        if ((timeout_state[i] & (TIMEOUT_USED | TIMEOUT_ARMED | TIMEOUT_PASSED)) == (TIMEOUT_USED | TIMEOUT_ARMED)) {
...
static void timeout_signal_handler(int signum __attribute__((unused)), siginfo_t *info, void *context __attribute__((unused)))
{
    ...
    for (i = 0; i < TIMEOUTS; i++)
        if ((timeout_state[i] & (TIMEOUT_USED | TIMEOUT_ARMED | TIMEOUT_PASSED)) == (TIMEOUT_USED | TIMEOUT_ARMED)) {
            ...
            if (seconds <= 0.0) {
                /* timeout [i] fires! */
                timeout_state[i] |= TIMEOUT_PASSED;
